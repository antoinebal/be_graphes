...
public class AStarTest {
	...
	@Test
	public void testAStarSansBellman() {
		...
		for (int origin = 0; origin < nodes.length; origin++) {
			for (int destination = 0; destination < nodes.length; destination++) {
				...
				if (origin == destination || !(AStar.run().isFeasible())) {
					solutions[origin][destination] = -1;
				} else {
					solutions[origin][destination] = AStar.run().getPath().getLength();
					// Test des résultats de AStar
	...
	@Test
	public void testAStarAvecBellman() {
		ShortestPathData data;
		AStarAlgorithm AStar;
