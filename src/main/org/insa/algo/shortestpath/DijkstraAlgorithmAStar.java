...
public class DijkstraAlgorithmAStar extends ShortestPathAlgorithm {


	public DijkstraAlgorithmAStar(ShortestPathData data) {
		super(data);
	}
	...
	protected ShortestPathSolution doRun() {
		...
		while (!tas.isEmpty() && !Fin) {
			...
			Point pointMin = nodeMin.getPoint();
			...
			for (Arc arc : nodeMin) {
				...
				if (labTab[arc.getDestination().getId()] == null) {
					...
					double vol_oiseau = pointMin.distanceTo(pointCourant);
					...
					labTab[arc.getDestination().getId()] = new LabelStar(false, (labTab[nodeMin.getId()].getCout() + w), vol_oiseau,
